...
USERNAME="${USERNAME:-smoke+$(date +%s)-${RANDOM}}"
...
request() {
  ...
  [ -n "$bearer" ] && args+=(-H "Cookie: session=$bearer")
  [ -n "$body" ] && args+=(-d "$body")
  local out
  out=$(curl "${args[@]}")
  HTTP_STATUS="${out##*$'\n'}"
...
step "registration"
request POST /auth/register "{\"username\":\"$USERNAME\", \"avatar\":\"avatar\", \"bio\": \"buid\", \"email\":\"$EMAIL\",\"password\":\"$PASSWORD\"}"
...
request GET /auth/me "" "$PENDING"
expect_status "pending session cannot access /me" 401

request POST /auth/mfa/totp "{\"code\":\"$(totp "$SECRET")\"}" "$PENDING"
...
request POST /auth/mfa/totp "{\"code\":\"000000\"}" "$PENDING"
